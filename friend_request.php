...
<body>
  ...
  ?><br><br><br><br>
    ...
    $query = "SELECT * FROM friends WHERE (userid1=$useridint or userid2=$useridint) AND stat=0 AND initiated<>$useridint";
    ...
    while($row = $result->fetch_assoc()){
        ...
        while($row1 = $result1->fetch_assoc()){
            $globaluserid = $row1['userid'];
                ...
                echo($result_image);
                    echo("</div>");
                    echo("<input type='hidden' value='$superuser' name='sentby'/>");
                    echo("<input type='hidden' value='$useridint' name='sentto'/>");
                    echo("<input type='hidden' value='$fid' name='id'/>");
                    echo("<input type='submit' value='Block' name='blockrequest' class='blockrequest' style='border:none;font-size:18px;float:right;color:#737373' >");
                    echo("<input type='submit' value='Reject' name='rejectrequest' class='rejectrequest' style='border:none;font-size:18px;float:right;color:#737373'>");                    
                    echo("<input type='submit' value='Accept' name='acceptrequest' class='acceptrequest' style='border:none;font-size:18px;float:right;color:#737373'>");
